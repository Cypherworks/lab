...
| `headscale_deb_url` | `{{ headscale_release_base }}/headscale_{{ headscale_version }}_linux_{{ headscale_arch }}.deb` | Full `.deb` download URL. |
...
1. Downloads the pinned `.deb` to `/tmp/headscale_{{ headscale_version }}.deb` (`0644`).
...
- The `.deb` is fetched over HTTPS from GitHub with no checksum verification in the role. Pin `headscale_version` deliberately.
